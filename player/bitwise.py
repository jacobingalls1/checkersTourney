...
notleft = (fullmask-left[0],
           fullmask-left[1])
notright = (fullmask-right[0],
           fullmask-right[1])
...
def boards2move(oldBoard, newBoard):
    ...
    if low&oldBoard:
        ...
        return (int2pos(high),int2pos(low))
...
def possMoves(board, player):
    ...
    #shift by 1 move additional move if jumping
    flJ = moves[4]((moves[0](fl&enemyPieces&notleft[player]))&emptySpace)
    frJ = moves[5]((moves[1](fr&enemyPieces&notright[player]))&emptySpace)
    blJ = moves[6]((moves[2](bl&enemyPieces&notleft[player]))&emptySpace)
    brJ = moves[7]((moves[3](br&enemyPieces&notright[player]))&emptySpace)
...
def evalBoard(board): # currently just counts pieces, kings are double, return positive for p0 advantage
    p = board2pieces(board)
    count = (2*bin(p[2]).count('1')+bin(p[0]).count('1')) - (2*bin(p[3]).count('1')+bin(p[3]).count('1')) 
    return count
...
'''
printBoard(notright[1])
printBoard(notleft[1])
